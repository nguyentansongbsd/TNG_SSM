...
namespace Plugin_SAP_CreateHDCS
{
    public class Plugin_SAP_CreateHDCS : IPlugin
    {
        ...
        private void Init()
        {
            ...
            if (((OptionSetValue)en["statuscode"]).Value != 100000001 && ((OptionSetValue)en["statuscode"]).Value != 100000011) return; //&& !en.Contains("bsd_bantacnghiepapprovedate")
            ...
            else if(this.en.Contains("bsd_datcoc"))
            {
                Entity enDatCoc = service.Retrieve(((EntityReference)this.en["bsd_datcoc"]).LogicalName, ((EntityReference)this.en["bsd_datcoc"]).Id, new ColumnSet(new string[] { "bsd_datcoccodesap", "bsd_mahethong" }));
            ...
            tracingService.Trace("Apitype: " + _apiType);
        ...
        {
            header2.auart = "ZDCS";
            header2.kunnr = this.customerCode;
            header2.bstkd = this.en.Contains("bsd_mahopongcoso") ? this.en["bsd_mahopongcoso"].ToString() : null;
            ...
            header2.zlhdct = getContractType();
        ...
        private async Task addHeaderSale()
        {
            ...
            headerSales.KETDAT = en.Contains("bsd_expecteddateofsigningspa") ? (RetrieveLocalTimeFromUTCTime((DateTime)en["bsd_expecteddateofsigningspa"],service)).ToString("yyyy-MM-dd") : null;
            headerSales.prsdt = this.en.Contains("bsd_datediscount") ? (RetrieveLocalTimeFromUTCTime((DateTime)en["bsd_datediscount"],service)).ToString("yyyy-MM-dd") : null;  //"2002-07-12";
            headerSales.vbegdat = this.en.Contains("bsd_signdatehdcs") ? (RetrieveLocalTimeFromUTCTime((DateTime)en["bsd_signdatehdcs"],service)).ToString("yyyy-MM-dd") : null; // "2022-08-01";
        ...
        {
            headerStatus.asttx = ((OptionSetValue)en["statuscode"]).Value != 100000011 ? "REL1" : "REL4"; // giam doc duyet
        ...
        private string getUnitName(EntityReference unit)
        {
            Entity encontact = service.Retrieve(unit.LogicalName, unit.Id, new ColumnSet(new string[1] { "fullname" }));
            return encontact["fullname"].ToString();
        }
        ...
        {
            if (!this.en.Contains("bsd_contracttypedeposite")) return null;
            var fetchXml = $@"<?xml version=""1.0"" encoding=""utf-16""?>
                <fetch top=""1"">
                  <entity name=""bsd_contracttype"">
                    <attribute name=""bsd_contracttypecodesap"" />
                    <filter>
                      <condition attribute=""bsd_contracttypecode"" operator=""eq"" value=""{((OptionSetValue)this.en["bsd_contracttypedeposite"]).Value}"" />
                    </filter>
                  </entity>
                </fetch>";
            var result = service.RetrieveMultiple(new FetchExpression(fetchXml));
            if (result == null || result.Entities.Count <= 0) return null;
            Entity enContractType = result.Entities[0];
            if (!enContractType.Contains("bsd_contracttypecodesap")) return null;
            return enContractType["bsd_contracttypecodesap"].ToString();
        }
        private async Task getListCoOwner()
        {
            tracingService.Trace("Start get coower");
            var fetchXml = $@"<?xml version=""1.0"" encoding=""utf-16""?>
                <fetch>
                  <entity name=""bsd_coowner"">
                    <filter>
                      <condition attribute=""bsd_reservation"" operator=""eq"" value=""{en.Id}""/>
                    </filter>
            ...
            tracingService.Trace("Done get coower");
        }
            ...
            var fetchXml = $@"<?xml version=""1.0"" encoding=""utf-16""?>
                <fetch>
                  <entity name=""bsd_summaryinstallment"">
                    ...
                    <filter>
                      <condition attribute=""bsd_quote"" operator=""eq"" value=""{this.en.Id}"" />
                    </filter>
                  </entity>
                </fetch>";
            var result = service.RetrieveMultiple(new FetchExpression(fetchXml));
